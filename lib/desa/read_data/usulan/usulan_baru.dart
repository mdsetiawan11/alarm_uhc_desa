...
class UsulanBaruPage extends StatefulWidget {
  const UsulanBaruPage({super.key});

  @override
  State<UsulanBaruPage> createState() => _UsulanBaruPageState();
}

class _UsulanBaruPageState extends State<UsulanBaruPage> {
  ...
  Future usulData() async {
    ...
    String nmdesa = (localStorage.getString('nama') ?? '');
    ...
    try {
      ...
      String usulUrl = "https://uhcdesa.jekaen-pky.com/api/add_usulan_baru";

      var response = http.MultipartRequest('POST', Uri.parse(usulUrl));
      response.fields['kddesa'] = kddesa;
      response.fields['nmdesa'] = nmdesa;
      response.fields['dati2'] = dati2;
      response.fields['nik'] = nik.text;
      response.fields['nama'] = nama.text;
      response.fields['status'] = 'DIUSULKAN';
